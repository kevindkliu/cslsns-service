...
public class JacksonMapper {
	...
	static {
		...
		/**
		 * 忽略的序列化设置。1、json串中存在的属性(且java对象中不存在的属性值)， 在转换为java对象时忽略。2、java对象存在(且没有实际的属性或值)
		 * 转换为json时忽略信息。
		 */
	...
	public static final <T> T jsonToClass(String json, Class<T> tClass) {
		...
		T req = null;
		try {
			req = tClass == String.class ? (T) json
					: tClass == JsonNode.class ? (T) toJsonNode(json)
							: Map.class.isAssignableFrom(tClass) ? jsonMapper.readValue(json, tClass)
									: classMapper.readValue(json, tClass);
		} catch (Exception e) {
	...
	public static final <T> T jsonNodeToClass(JsonNode jsonNode, Class<T> tClass) {
		...
		try {
			req = tClass == String.class ? (T) jsonNode.toString()
					: tClass == JsonNode.class ? (T) jsonNode
							: Map.class.isAssignableFrom(tClass) ? jsonMapper.convertValue(jsonNode, tClass)
									: classMapper.convertValue(jsonNode, tClass);
		} catch (Exception e) {
